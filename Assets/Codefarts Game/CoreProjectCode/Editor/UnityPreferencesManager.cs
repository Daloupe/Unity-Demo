        ...
        private void ShowSelectSaveFileDialog()
        {
            ...
            if (!string.IsNullOrEmpty(previousFullPath) && string.Compare(previousFullPath, path, StringComparison.InvariantCultureIgnoreCase) != 0)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    return;
                    //Debug.LogWarning("The existing settings file threw an exception when trying to copy it to the new destination.");
                }
            }
            ...
            {
                Debug.LogException(ex);
                //  Debug.LogWarning("Exception occurred saving empty settings file.");
            }
